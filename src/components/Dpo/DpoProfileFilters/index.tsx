...
import { useGetLastBlock } from 'hooks/useBlocks'
...
export default function DpoProfileFilters({ unfilteredDpos, setFilteredDpos }: DpoAllFilterProps) {
  ...
  }, [unfilteredDpos, unfilteredDpos.length, filteredState, filteredAsset, lastBlock])

  // Final after all Filters, Sorts and Searches
  useEffect(() => {
    // Sort the filtered results
    let sortedDpos = primaryFilteredDpos
    if (sortBy.length > 0) {
  ...
  }, [primaryFilteredDpos, primaryFilteredDpos.length, sortBy, sortBy.length, sortedOptions, sortedOptions.length])
